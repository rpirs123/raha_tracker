...
func RunPrompts(r *util.Record) {
	// log for the user
	fmt.Println("\033[32m", r.Timestamp, ":", r.Supplier, "on kandnud raha. Summa:", r.Amount, r.Currency, "\033[0m")
	...
	if !iniutil.SupplierInConfig(r.Supplier) {
		...
		if choice == "jah" {
			...
			r.Category = PromptCategoryName(r.SectionName)
...
func PromptCategoryName(vendor string) string {
	// using bufio reader here due to promptui bugging with back to back prompts
	reader := bufio.NewReader(os.Stdin)

	fmt.Printf("Sisesta kategooria mille alla %s salvestada\n", vendor)
